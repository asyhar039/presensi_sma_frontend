...
/**
 * ResetPasswordModal - UI Slicing for Student Password Management.
 *
 * NOTE: API Integration is pending Backend endpoint creation (e.g. POST /students/:id/reset-password).
 * Currently uses local demo state for presentation purposes.
 */
const ResetPasswordModal = ({ open, student, onClose }) => {
  ...
  const studentName = student?.nama_lengkap || student?.name || 'Siswa';
  ...
  const handleSubmit = (e) => {
    ...
    // TODO: Replace demo submit with backend reset-password endpoint
    // Backend currently requires full student update payload via PUT /students/{id}
    setTimeout(() => {
      ...
      <Modal open={open} onClose={handleCloseAll} size="sm">
        <div className="flex flex-col items-center text-center p-4">
          ...
          <p className="text-sm text-slate-500 mb-6">
            Kata sandi untuk akun <span className="font-semibold text-slate-700">{studentName}</span> telah divalidasi dan berhasil disimpan.
          </p>
          <Button
            variant="primary"
  ...
  return (
    <Modal open={open} title="Manajemen Password Siswa" onClose={handleCloseAll}>
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-start gap-3 rounded-2xl bg-indigo-50/70 p-4 border border-indigo-100/60">
          ...
          <div className="text-sm text-indigo-900">
            ...
            <div className="font-bold text-slate-900 text-base">{studentName}</div>
          </div>
        </div>
